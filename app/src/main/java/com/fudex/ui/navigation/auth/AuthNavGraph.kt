package com.fudex.ui.navigation.auth
        ...
        composable(Destination.LOGIN.route) {
            LoginScreen(
                navToHome = { role ->
                    when (role) {
                        "Client" -> {
                            navController.navigate(Destination.CLIENT_GRAPH.route) {
                                popUpTo(Destination.AUTH_GRAPH.route) { inclusive = true }
                            }
                        }
                        "Restaurant" -> {
                            navController.navigate(Destination.RESTAURANT_GRAPH.route) {
                                popUpTo(Destination.AUTH_GRAPH.route) { inclusive = true }
                            }
                        }
                        "Delivery" -> {
                            navController.navigate(Destination.DELIVERY_GRAPH.route) {
                                popUpTo(Destination.AUTH_GRAPH.route) { inclusive = true }
                            }
                        }
